...
local symbol_at = require("gdshader_blink.semantic.symbol_at")
...
function M.hover()
    ...
    if ft ~= "gdshader" and ft ~= "gdshaderinc" then
        return
    end
...
function M.attach(bufnr)
    ...
    if vim.b[bufnr].gdshader_blink_hover_attached then
        return
    end

    vim.b[bufnr].gdshader_blink_hover_attached = true
    ...
    vim.keymap.set("n", "<Plug>(gdshader-blink-hover)", M.hover, {
        buffer = bufnr,
    ...
    local existing = vim.fn.maparg("K", "n", false, true)
...
function M.setup()
    ...
    local group = vim.api.nvim_create_augroup("GDShaderBlinkHover", {
        clear = true,
    })

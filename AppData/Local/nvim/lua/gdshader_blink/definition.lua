...
function M.attach(bufnr)
    ...
    if vim.b[bufnr].gdshader_blink_definition_attached then
        return
    end

    vim.b[bufnr].gdshader_blink_definition_attached = true
    ...
    vim.keymap.set("n", "<Plug>(gdshader-blink-definition)", M.goto_definition, {
        buffer = bufnr,
    ...
    local existing = vim.fn.maparg("gd", "n", false, true)
...
function M.setup()
    ...
    local group = vim.api.nvim_create_augroup("GDShaderBlinkDefinition", {
        clear = true,
    })
